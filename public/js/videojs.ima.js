(function (global, factory) {
    ...
    AdUi.prototype.assignControlAttributes = function (element, controlName) {
        ...
        element.className =
            this.controlPrefix + controlName + " " + controlName;
    };
     ...
     */
    const PlayerWrapper$2 = function (
        player,
        adsPluginSettings,
        daiController
    ) {
    ...
    SdkImpl$2.prototype.onStreamEvent = function (event) {
        switch (event.type) {
            ...
            case google.ima.dai.api.StreamEvent.Type.ERROR:
                window.console.warn(
                    "Error loading stream, attempting to play backup " +
                        "stream. " +
                        event.getStreamData().errorMessage
                );
                this.daiController.onErrorLoadingAds(event);
                if (this.daiController.getSettings().fallbackStreamUrl) {
                    this.loadurl(
                        this.daiController.getSettings().fallbackStreamUrl
                    );
                }
                break;
            ...
            case google.ima.dai.api.StreamEvent.Type.AD_BREAK_ENDED:
                ...
                if (
                    this.snapForwardTime &&
                    this.snapForwardTime > this.vjsPlayer.currentTime()
                ) {
    ...
    SdkImpl$2.prototype.loadUrl = function (streamUrl) {
        this.vjsPlayer.ready(
            function () {
                const streamFormat =
                    this.daiController.getSettings().streamFormat;
                this.vjsPlayer.src({
                    src: streamUrl,
                ...
                const bookmarkTime =
                    this.daiController.getSettings().bookmarkTime;
                if (bookmarkTime) {
                    const startTime =
                        this.streamManager.streamTimeForContentTime(
                            bookmarkTime
                        );
    ...
    SdkImpl$2.prototype.requestStream = function () {
        ...
        if (streamType === "vod") {
            ...
            streamRequest.contentSourceId =
                this.daiController.getSettings().cmsId;
    ...
    const DaiController = function (player, options) {
        ...
        this.playerWrapper = new PlayerWrapper$2(
            player,
            adsPluginSettings,
            this
        );
    ...
    DaiController.prototype.warnAboutDeprecatedSettings = function () {
        ...
        deprecatedSettings.forEach((setting) => {
            if (this.settings[setting] !== undefined) {
                console.warn(
                    "WARNING: videojs.imaDai setting " +
                        setting +
                        " is deprecated"
                );
            }
        ...
        constructor(streamFormat, assetKey) {
            ...
            if (streamFormat !== "hls" && streamFormat !== "dash") {
                window.console.error(
                    "VodStream error: incorrect streamFormat."
                );
                return;
            } else if (streamFormat === "dash") {
                window.console.error(
                    "streamFormat error: DASH streams are not" +
